"""
DeepWorld v5.3 — Multi-Model Adapter (NVIDIA NIM, text-mode tools)
==================================================================
Routes agent LLM calls to NVIDIA NIM free-tier models.

Backend: https://integrate.api.nvidia.com/v1 (NVIDIA_API_KEY env / secret)
Pool: DEFAULT_MODEL_POOL in v4/config/__init__.py (locally verified 2026-09-20)

NVIDIA NIM free models do NOT support native OpenAI tool calling, so tools
are injected as text into the last user message and the JSON reply is parsed
back (see _parse_tool_from_text + TOOL_KEYWORDS fallback).

Retry policy: 429/5xx/timeout = transient (retry 3x, backoff). 400/401/403/
404/410 = permanent for this model (fail fast — don't burn minutes retrying
a gated or EOL model; the agent idles this tick and the pool gets culled).
"""
...
def _is_transient_nim_error(e: Exception) -> bool:
    ...
    False (fail fast) for 400/401/403/404/410 — gated, EOL, or bad request
    will never succeed on retry; the agent idles this tick instead.
    """
    text = f"{type(e).__name__}: {e}"
    if any(m in text for m in ("429", "500", "502", "503", "504",
                               "rate-limited", "Rate limit", "rate_limit",
                               "Timeout", "timeout", "timed out",
                               "Connection", "connection", "RemoteDisconnected",
                               "network_error", "temporarily", "overloaded")):
        return True
    return False
...
class MultiModelAdapter:
    ...
    def create_completion(self, model: str, messages: list, tools: list = None,
        ...
        if backend_type in ("openai", "deepseek_api"):
            ...
            actual_model = model if (model and model != "nemotron") else "nvidia/nemotron-3-super-120b-a12b"
            ...
            for attempt in range(3):
                try:
                    ...
                    if not _is_transient_nim_error(e):
                        break  # gated/EOL/auth — fail fast, don't burn minutes
                    import time as _time2
                    _time2.sleep(5 * (attempt + 1))
            import sys
            print(f"[DEEPWORLD] NVIDIA API error for {actual_model}: {type(last_err).__name__}: {last_err}", file=sys.stderr)
            raise last_err if last_err is not None else RuntimeError(f"NVIDIA call failed for {actual_model}")
        else:
...
TOOL_KEYWORDS = {
    ...
    # v5.1 repo tools — BEFORE single-word v3 keywords to win multi-word matches
    "write code": "write_code", "write a file": "write_code", "write file": "write_code",
    ...
    "vote on proposal": "vote_contribution",
    "write doc": "document_code", "improve doc": "document_code",
    "document": "document_code", "doc": "document_code", "docs": "document_code",
    "view repo": "view_repo_stats", "repo stats": "view_repo_stats",
    "view files": "view_repo_files", "list files": "view_repo_files",
    "see files": "view_repo_files", "show files": "view_repo_files",
    "read the file": "read_repo_file", "read file": "read_repo_file", 
    "read code": "read_repo_file", "examine file": "read_repo_file",
    "collaborate with": "collaborate", "co-author": "collaborate",
    "accept collab": "accept_collaboration", "accept invitation": "accept_collaboration",
    # v3/v4 tools
